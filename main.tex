...
Each evaluation run is a single episode consisting of a briefing, a primary
campaign, and a general-election campaign. At the start of the episode, the
model receives the voter vignette, its party registration, and the rules of the
inspection interface. The vignette defines the goal of choosing the candidate
...
visible item or stop searching and cast a vote.

During the primary stage, cue labels arrive in a rolling window and compete for
...
while the model is reading. The primary stage ends when the model votes for an
in-party candidate or when the stage budget is exhausted. The environment then
announces the two nominees and resets the stream for the general election,
where the model faces the same inspection-and-vote loop over the finalists.
Any written justification is collected only after the vote is recorded, so the
process trace reflects pre-decision search rather than post hoc explanation.
...
with its stage, cue type, candidate target, and whether the information was new
or revisited. These logs are converted into the Mouselab measures of search
depth, comparability, and sequence for the primary and general election
separately, and the final votes are scored against the fully informed
benchmark. The resulting data pair outcome quality with information-acquisition
...
\node[process] (brief) {Briefing: provide the voter vignette, party registration,
and inspection rules.};
\node[process, below=of brief] (primary) {Primary campaign: inspect one visible cue
at a time while the rolling window continues to advance.};
\node[decision, below=8mm of primary] (primarystop) {Vote cast or stage budget
exhausted?};
\node[process, below=8mm of primarystop] (transition) {Record the primary vote,
announce the nominees, and reset the cue stream.};
\node[process, below=of transition] (general) {General election: repeat the same
inspection-and-vote loop over the two finalists.};
\node[decision, below=8mm of general] (generalstop) {Vote cast or stage budget
exhausted?};
\node[process, below=8mm of generalstop] (post) {Record the final vote, collect any
optional written justification, and log every inspection action.};
\node[process, below=of post] (score) {Score correctness against the fully informed
benchmark and compute depth, comparability, and sequence of search.};
\coordinate (primaryloop) at ([xshift=10mm]primary.east);
\coordinate (primaryreturn) at (primaryloop |- primarystop.east);
\coordinate (generalloop) at ([xshift=10mm]general.east);
\coordinate (generalreturn) at (generalloop |- generalstop.east);

\draw[line] (brief) -- (primary);
\draw[line] (primary) -- (primarystop);
\draw[line] (primarystop) -- node[right, font=\footnotesize] {Yes} (transition);
\draw[line] (transition) -- (general);
\draw[line] (general) -- (generalstop);
\draw[line] (generalstop) -- node[right, font=\footnotesize] {Yes} (post);
\draw[line] (post) -- (score);
\draw[line] (primarystop.east) -- node[midway, above, font=\footnotesize]
      {No: inspect next cue} (primaryreturn) -- (primaryloop) -- (primary.east);
\draw[line] (generalstop.east) -- node[midway, above, font=\footnotesize]
      {No: inspect next cue} (generalreturn) -- (generalloop) -- (general.east);
\end{tikzpicture}
...
All measures are computed separately for the primary and general-election
stages, with the relevant choice set defined as the candidates who are actually
eligible at that stage. In the primary, this means that process measures are
restricted to in-party candidates; inspections of out-party material are logged
but do not contribute to the primary search metrics. Repeated openings of the
same cue are retained in the raw trace, but they count as revisits rather than
new information.
...
stage, the environment defines a fully informed utility ranking over the
eligible candidates from the assigned voter profile and the hidden cue values. A
vote is scored as correct if the model selects the candidate with the highest
fully informed utility; ties are treated as jointly correct. We report
correctness as a binary outcome at the primary and general-election stages and
retain the full trace so that accurate choices reached by systematic
comparative search can be distinguished from equally accurate choices reached
by narrow heuristic search. This separation between process and outcome is
central to Tool-Lab: it lets us evaluate not only whether a model reaches the
right answer, but how it used its limited inspection opportunities to get
there.
